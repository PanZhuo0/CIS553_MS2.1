...
void LSMessage::LSAInfo::Serialize(Buffer::Iterator& start) const{
	//### write header
	start.WriteHtonU32(originNode);
	start.WriteHtonU32(sequenceNumber);
	start.WriteHtonU32(neighbors.size()); // ### FOR deserialize can know how many neighbor | size of neighbors

	//### write data/load
	for(auto& n:neighbors){
		start.WriteHtonU32(n.first);
		start.WriteHtonU32(n.second);
	}
}; // serilize function implement ed 



//### deserialize
uint32_t LSMessage::LSAInfo::Deserialize(Buffer::Iterator& start){
	//originNode deserialize
	  originNode = start.ReadNtohU32();
	//SEQ deserialize
	  sequenceNumber = start.ReadNtohU32();
	//map deserialize
	  uint32_t count = start.ReadNtohU32(); // ### as i think ,first get sizeof map second unserialize it
	  for (uint32_t i = 0; i < count; i++) {
	    uint32_t node = start.ReadNtohU32();
	    uint32_t cost = start.ReadNtohU32();
	    neighbors[node] = cost;
	  }
	  //return a serialized size 
	  return GetSerializedSize();
}; // Deserilize impelement ed

//### SetLSA 
void LSMessage::SetLSA(uint32_t originNode, uint32_t sequenceNumber, const std::map<uint32_t, uint32_t>& neighbors) {
  m_messageType = LSA;
  m_message.lsa.originNode = originNode;
  m_message.lsa.sequenceNumber = sequenceNumber;
  m_message.lsa.neighbors = neighbors;
}

//### GETLSA
LSMessage::LSAInfo LSMessage::GetLSA() const{
